...
public class Producto{
    ...
    public DefaultTableModel leer_tabla_2() throws SQLException{
    DefaultTableModel tabla = new DefaultTableModel();
    try{
    cn = new Conexion();
    cn.abrir_conexion();
    String query = "SELECT idproductos,descripcion,precio_venta,precio_costo,existencia FROM productos;";
    ResultSet consulta = cn.conexionBD.createStatement().executeQuery(query);
    String encabezado[] = {"id","cantidad","descripcion","precio_unitario","precio_costo","existencia"};
    tabla.setColumnIdentifiers(encabezado);
      String datos[] = new String[4];
      while (consulta.next()){
          datos[0] = consulta.getString("idproductos");
