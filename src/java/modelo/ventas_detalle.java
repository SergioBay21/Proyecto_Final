...
public class ventas_detalle {
    ...
    Conexion cn;
    int id_producto, p_unitario,existencia;

    String cantidad;

    public ventas_detalle() {}

    public ventas_detalle(int id_producto, int p_unitario, int existencia, String cantidad) {
    ...
    public int leer_venta() {
        int id_venta_ = 0;
        try {
            cn = new Conexion();
            String query = "SELECT max(idventa) as idventa FROM ventas;";
            cn.abrir_conexion();
            ResultSet consulta = cn.conexionBD.createStatement().executeQuery(query);
            consulta.next();
            id_venta_ = Integer.valueOf(consulta.getString("idventa"));
            cn.cerrar_conexion();
        } catch (SQLException ex) {
            System.out.println(ex.getMessage());
        }
